...
### WPF DB바인딩 - [디자인](./day01/Day01Wpf/WpfBasicApp01/MainWindow.xaml) / [소스](./day01/Day01Wpf/WpfBasicApp01/MainWindow.xaml.cs)
1. 프로젝트 생성
...
6. DB연결
    ...
    5. DB명령 객체(SqlCommand) : 쿼리를 컨트롤하는 객체
    ...
    6. 데이터어댑터(`SqlDataAdapter`) : 연결 이후 데이터 처리를 쉽게 도와주는 객체
...
- **MVVM** : Model-View-ViewModel 패턴
    ...
    - Model은 당연히 DB요청, 응답
    - 연결방식이 MVC와 다름 
    - 전통적인 C# 방식은 사용자가 이벤트 발생 시키기 때문에 발생 시기를 바로 알 수 있음
...
9. MainView.xaml 컨트롤에 바인딩 작업
    - 전통적인 C# 방식 : x:Name 사용(비하인드 사용 필요), 마우스 이벤트 추가
...
### MVVM Framework
- MVVM 개발자체가 어려움. 초기 개발 시 MVVM 템플릿을 만드는데 시간이 많이 소요. 난이도 있음
...
5. MainViewModel 클래스 생성(이름이 고정됨) - [소스](./day02/Day02Wpf/WpfBasicApp01/ViewModels/MainViewModel.cs)
    - MainView에 속하는 ViewModel은 반드시 MainViewModel 라는 이름을 써야함
...
https://github.com/user-attachments/assets/2d78ee82-59aa-4d42-993f-166f47e642fd
...
10. 실행 시 텍스트박스에 포커스 가도록 처리
    ...
    https://github.com/user-attachments/assets/657c93e5-18c8-4c28-b4a6-e70030197fcb
...
4. 즐겨찾기 DB연동
    ...
    2. movieitems 테이블 생성. 컬럼은 MovieItems.cs 속성과 동일
    3. INSERT, UPDTAE, DELETE 작업
...
7. 결과 화면
    ...
    https://github.com/user-attachments/assets/1278fafd-acd8-4e7b-ab42-03d8a53b3ab1
...
- MQTT와 유사한 기능을 하는 기술
    - `Redis`, `Apache Kafka`, **RabbitMQ**, ZeroMQ, Socket 통신 직접개발 
...
1. MQTT 브로커 설치
    ...
    - 설치 후 서비스에서 서비스 중지
2. 모스키토 설정파일 수정
    - mosquitto.conf 문서 에디터 관리자모드로 오픈
    - #listener -> listenner 1883 으로 변경
...
3. C# MahApps.Metro 사용 MQTT 데이터 Subscribe 앱
    - CommunityToolkit.Mvvm
    - MahApps.Metro
    - MahApps.Metro.IconPacks
    - Newtonsoft.Json
...
6. MqttPub.py와 Publish된 IoT 데이터 WPF에서 Subscribe 예제
    ...
    https://github.com/user-attachments/assets/2cb83301-9038-489a-a349-cf19135b780b
